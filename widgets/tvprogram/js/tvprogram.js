...
/* global vis, $, systemDictionary, jQuery*/
...
vis.binds["tvprogram"] = {
    ...
    search: {
        ...
        createWidget: function (widgetID, view, data, style) {
            ...
            const heightrow = parseInt(data.heightRow) || 35;
            const broadcastfontpercent = parseInt(data.broadcastfontpercent) || 75;
            const highlightcolor = data.highlightcolor || "yellow";
            const showpictures = data.showpictures || false;

            const dialogwidthpercent = data.dialogwidthpercent / 100 || 0.9;
            const dialogheightpercent = data.dialogheightpercent / 100 || 0.9;
            ...
            if (tvprogram_oid && !this.bound[tvprogram_oid][widgetID]) {
                ...
                if (1 || !vis.editMode) {
                    ...
                    vis.binds["tvprogram"].bindStates2($div, [
        ...
        onSubmitSearch: function (widgetID, view, data, style, evt) {
            ...
            if (channelfilter.length == 0) channelfilter = this.visTvprogram.channels.reduce((acc, el, i) => {
                if (i < 4) acc.push(el.id);
                return acc;
            }, []);
    ...
    control: {
        ...
        createWidget: function (widgetID, view, data, style) {
            ...
            if (!this.programdata[tvprogram_oid][widgetID]) this.programdata[tvprogram_oid][widgetID] = [];

            if (time == "") {
                if (
                    Object.keys(this.programdata[tvprogram_oid][widgetID]).length == 0 ||
                    this.programdata[tvprogram_oid][widgetID].some(el => new Date(el.endTime) <= new Date())
                ) {
                    this.visTvprogram.getServerBroadcastNow(
                        instance,
                        channelfilter,
                        function (widgetID, view, data, style, serverdata) {
                ...
                if (
                    ...
                    this.visTvprogram.getServerBroadcastDate(
                        instance,
                        channelfilter,
                        this.parseTime(time),
                        function (widgetID, view, data, style, serverdata) {
            ...
            if (!this.timer[widgetID]) {
                this.timer[widgetID] = setInterval(() => {
                    const programdata = this.programdata[tvprogram_oid][widgetID].reduce((acc, el) => {
                        acc.push(el.events[0]);
                        return acc;
                    }, []);
    ...
    favorites: {
        ...
        createWidget: function (widgetID, view, data, style) {
            ...
            const maxfavorites = data.maxfavorites || 10;
            const highlightcolor = data.highlightcolor || "yellow";
            const channelname = data.channelname || false;
            ...
            if (JSON.stringify(this.oldFavoritesConf) != JSON.stringify(favorites)) this.visTvprogram.getFavoritesData(instance, favorites, function (widgetID, view, data, style, tvprogram_oid, serverdata) {
                this.oldFavoritesConf = favorites;
    ...
    time1: {
        ...
        createWidget: function (widgetID, view, data, style) {
            ...
            //vis.conn.getConfig(undefined, undefined, undefined, undefined, undefined);
            ...
            if (!this.today[widgetID]) this.today[widgetID] = {
                today: new Date(),
                prevday: null
            };
            if (!this.scroll[widgetID]) this.scroll[widgetID] = {
                time: new Date(0),
                position: 0,
                marker: 0,
                timeout: null,
            ...
            if (!this.viewday[widgetID]) this.viewday[widgetID] = {
                viewday: datestring,
                prevday: null
            };
            ...
            const headerfontpercent = data.headerfontpercent || 125;
            const broadcastfontpercent = data.broadcastfontpercent || 75;
        ...
        onclickChannel: function (widgetID, instance, tvprogram_oid) {
            ...
            let channelfilter = this.visTvprogram.getConfigChannelfilter(tvprogram_oid);
            if (channelfilter.length == 0) channelfilter = channels.reduce((acc, el, i) => {
                if (i < 4) acc.push(el.id);
                return acc;
            }, []);
        ...
        getButtonHeader: function (datestring) {
            ...
            hh.push('<li class="tv-item dateinfo">' + new Date(datestring).toLocaleDateString(navigator.language, {
                weekday: "short"
    ...
    getServerBroadcast: function (instance, eventid, viewdate, callback) {
        ...
        this.sendTo(instance, "getServerBroadcast", {
            eventid: eventid,
            viewdate: viewdate
        }, function (data) {
    ...
    sendTo: function (instance, command, data, cb) {
        console.log(`sendTo ${instance} ${command} ${JSON.stringify(data)}`);
        //this.sendTo(instance,"",{},function(){});
        if (parseInt(vis.version) >= 2) {
            vis.conn.sendTo(instance, command, data, cb);
        } else {
            vis.conn._socket.emit("sendTo", instance, command, data, function (result) {
                cb && cb(result);
            });
        }
    },
    events: {},
    serverdata: {},
    getServerDataOld: function (instance, widgetID, dataname, callback) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.events, name)) {
            if (!this.events[name].find((el) => el.key == widgetID)) this.events[name].push({
                key: widgetID,
                cb: callback
            });
        ...
        this.sendTo(instance, "getServerData", dataname, function (data) {
            if (data != "error" && data != "nodata") {
    ...
    getServerData: function (instance, widgetID, dataname, callback) {
        ...
        vis.conn.readFile("/" + instance + "/" + dataname + ".json", (err, data, filename) => {
            if (err) {
                console.error(`Instance: ${instance} WidgetID:${widgetID} File: ${filename} Error: ${err}`);
            } else {
                console.log(`Instance: ${instance} File: ${filename} loaded`);
            }
            this.serverdata[name] = JSON.parse(data);
            if (!Object.prototype.hasOwnProperty.call(this.events, name)) return;
            for (let i = 0; i < this.events[name].length; i++) {
                this.events[name][i].cb(this.serverdata[name]);
            }
            delete this.events[name];
        }, true);
    },
    getServerTVProgramOld: function (instance, widgetID, dataname, callback) {
        ...
        vis.sendTo(instance, "getServerTVProgram", dataname, function (data) {
            if (data != "error" && data != "nodata") {
    ...
    getServerTVProgram: function (instance, widgetID, dataname, callback) {
        ...
        vis.conn.readFile("/" + instance + "/program/" + dataname + ".json", (err, data, filename) => {
            if (err) {
                console.error(`Instance: ${instance} WidgetID:${widgetID} File: ${filename} Error: ${err}`);
            } else {
                console.log(`Instance: ${instance} File: ${filename} loaded`);
    ...
    getFavoritesData: function (instance, favorites = [], callback) {
        ...
        this.sendTo(instance, "getFavoritesData", favorites, function (data) {
            if (data != "error" && data != "nodata") {
    ...
    getServerInfo: function (instance, callback) {
        ...
        this.sendTo(instance, "getServerInfo", {}, function (data) {
    ...
    getServerBroadcastNow: function (instance, channelfilter, callback) {
        ...
        this.sendTo(instance, "getServerBroadcastNow", channelfilter, function (data) {
            if (data != "error" && data != "nodata") {
    ...
    getServerBroadcastDate: function (instance, channelfilter, date, callback) {
        ...
        this.sendTo(instance, "getServerBroadcastDate", {
            channelfilter: channelfilter,
            date: date
        }, function (data) {
            if (data != "error" && data != "nodata") {
    ...
    getServerBroadcastFind: function (instance, obj, callback) {
        ...
        this.sendTo(instance, "getServerBroadcastFind", obj, function (data) {
            if (data != "error" && data != "nodata") {
    ...
    setValueAck: function (instance, id, value) {
        ...
        this.sendTo(instance, "setValueAck", {
            id: id,
            value: value
        }, function () { }.bind(this));
    },
    ...
    bindStates2: function (elem, bound, change_callback) {
        ...
        if (parseInt(vis.version) >= 2) {
            ...
            vis.conn.getStates(bound, function (error, states) {
                ...
                const o = {};
                for (let i = 0; i < bound.length; i++) {
                    o[`${bound[i]}.val`] = states[bound[i]]["val"];
                }
                vis.states.attr(o);
                for (let i = 0; i < bound.length; i++) {
                    vis.states.bind(bound[i] + ".val", change_callback);
                }
                $div.data("bound", bound);
                $div.data("bindHandler", change_callback);
            }.bind({
