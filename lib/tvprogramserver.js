...
*/        
    ...
    this.getServerBroadcastFindMsg = function(msg) {
        ...
        var data='error';
        if (typeof msg.message=="object") {
            var channelfilter = msg.message.channelfilter||[];
            var categoryfilter = msg.message.categoryfilter||[];
            var textfilter = msg.message.textfilter||"";
            var textfilter = textfilter.trim().toLowerCase();
            var dstart = new Date(msg.message.datefrom);
            var dend = new Date(msg.message.datetill);
            var dstartfind = new Date(msg.message.datefrom);
            var dendfind = new Date(msg.message.datetill);
            dstart.setDate(dstart.getDate()-1);
            dend.setDate(dend.getDate()+1);
            var data=[],i;
            while(dstart <= dend){
                ...
                if (this.tvdata.program[datum]) {
                    this.tvdata.program[datum].map((el)=>{
                        if (    (i=channelfilter.indexOf(el.channel))>-1 &&
                                ...
                                ((textfilter=="")?true:el.title.toLowerCase().includes(textfilter))
                            ) {
                            if (!data[i]) data[i]={};
                            if (!data[i].events) data[i].events=[];
                            data[i].channel=el.channel;
            ...
            if (categoryfilter.length>0) {
                data=data.map((ch,i,arr)=>{
                    ch.events=ch.events.filter( (el)=>categoryfilter.includes(el.content.category));
                    return ch;
                });
                data=data.filter((ch)=>ch.events.length>0);
            }
        }
        if (msg.callback) this.adapter.sendTo(msg.from, msg.command, data, msg.callback);
    }
