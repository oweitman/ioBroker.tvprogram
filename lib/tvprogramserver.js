...
function tvprogramclass(adapter) {
    ...
    this.processMessages = function(msg) {
        ...
        if (msg.command === 'getServerBroadcastsNow') {
            this.log.debug('send getServerBroadcastsNow');
            this.getServerBroadcastsNowMsg(msg);
        }
    }
    ...
    this.getServerBroadcastsNowMsg = function(msg) {
        ...
        if (typeof msg.message=="object") {
            ...
            if (this.tvdata.program[datum]) {
                data = this.tvdata.program[datum].reduce(function(acc,el,index,arr) {
                    var prevEl;
                    if (index>0) prevEl = arr[index-1];
                    if (new Date(el.startTime)<=new Date() && new Date(el.endTime)>=new Date() && channelfilter.includes(el.channel)) acc.push(el);
                    if (prevEl) if (new Date(prevEl.startTime)<=new Date() && new Date(prevEl.endTime)>=new Date() && channelfilter.includes(prevEl.channel)) acc.push(el);
                    return acc;
                },[]);
            }
        }
    ...
    this.getFavoritesDataMsg = function(msg) {
        ...
        if (typeof msg.message=="object") {
            ...
            while(dstart <= dend){
                ...
                var datum = this.formatedDate(dstart)
                if (this.tvdata.program[datum]) {
