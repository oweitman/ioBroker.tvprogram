...
*/
    ...
    this.getFavoritesDataMsg = function (msg) {
        ...
        if (msg.callback) this.adapter.sendTo(msg.from, msg.command, result.sort((a, b) => +new Date(a.startTime) - +new Date(b.startTime)) || "errror", msg.callback);
    };
    ...
    this.getData = async function (tvs) {
        ...
        dend.setDate(dend.getDate() + 5);
        while (dstart <= dend) {
            const datum = this.formatedDate(dstart);
            this.getProgram(dstart, (data) => {
                if (data === "error") {
                ...
                if (JSON.stringify(this.tvdata.program[datum]) !== JSON.stringify(data.events)) {
                    ...
                    this.writeFile("/program/" + datum + ".json", data.events);
        ...
        while (dstart >= dend) {
            const data = await this.readFileSync("program/" + this.formatedDate(dstart) + ".json");
            if (data === "error") {
                dstart.setDate(dstart.getDate() - 1);
                continue;
            }
    ...
    this.writeFileOld = function (filename, data, callback) {
    ...
    this.writeFile = function (filename, data, callback) {
        if (typeof data != "string") data = JSON.stringify(data);
        this.adapter.writeFile(this.adapter.namespace, filename, data, (err) => {
            if (err) {
                this.log.debug("Error saving file " + filename + "! " + err.message);
            } else {
                this.log.debug("The file " + filename + " has been saved!");
    ...
    this.readFileOld = function (filename, callback) {
    ...
    this.readFile = function (filename, callback) {
        this.adapter.readFile(this.adapter.namespace, filename, (err, data) => {
            if (err) {
                this.log.debug("Error loading file " + filename + "! " + err.message);
                callback("error");
            } else {
    ...
    this.readFileSyncOld = function (filename) {
        let data;
        try {
    ...
    this.readFileSync = async function (filename) {
        let data;
        try {
            const ret = await this.adapter.readFileSync(this.adapter.namespace, filename);
            data = JSON.parse(ret.file);
        } catch {
            data = "error";
        }
        return data;
    };
    this.removeFilesOld = function () {
    ...
    this.removeFiles = function () {
        this.adapter.readDir(this.adapter.namespace, "/program/", (err, files) => {
            files.forEach(file => {
                if ((+new Date() - +new Date(file.file.split(".")[0])) / (1000 * 60 * 60 * 24) > 5) this.unlinkFile("/program/" + file.file);
            });
        });
    };
    this.unlinkFile = async function (filename) {
        try {
            await this.adapter.unlinkAsync(this.adapter.namespace, filename);
        } catch (error) {
    ...
    this.log.silly = (s) => {
        if (this.logsilly) adapter.log.silly(s);
    };
    this.log.debug = (s) => {
        if (this.logdebug) adapter.log.debug(s);
    };
    this.log.error = (s) => {
        this.log.error(s);
    };
