...
const { sourceIdentity, sourceChanged, stableId, nextDownload, parseXmltv, hasUsableGuide } = require('./epg');
const tvfueralle = require('./tvfueralle');
...
function tvprogramclass(adapter) {
    ...
    this.doObserver = async function () {
        ...
        try {
            const tvs = await this.getTVDatapointsAsync();
            await this.newTVDatapointsAsync(tvs);
            await this.loadCache(tvs);
            if (this.adapter.config.source === 'iptv-epg') {
                if (hasUsableGuide(this.tvdata)) {
                    this.scheduleIptv(tvs);
                } else {
                    this.logdebug('No usable IPTV-EPG cache found; downloading now');
                    await this.runIptv(tvs, this.formatedDate(new Date()));
                }
            } else {
                await this.getDataAsync(tvs);
                if (!hasUsableGuide(this.tvdata)) {
                    throw new Error('TV für alle returned no usable guide data');
                }
                if (!this.stopping) {
            ...
            this.logerror(`TV programme update failed: ${String(error)}`);
            if (!this.stopping) {
    ...
    this.removeFiles = function () {
        this.logdebug('removeFiles');
        const cutoff = new Date();
        cutoff.setHours(0, 0, 0, 0);
        cutoff.setDate(cutoff.getDate() - 5);
        fs.readdir(`${dataDir}program/`, (err, files) => {
            if (err) {
                this.logerror(`Cannot list programme cache: ${err.message}`);
                return;
            }
            files.forEach(file => {
                if (/^\d{4}-\d{2}-\d{2}\.json$/.test(file) && file.slice(0, 10) < this.formatedDate(cutoff)) {
                    fs.unlink(`${dataDir}program/${file}`, error => {
                        if (error) {
                            this.logerror(`Cannot remove ${file}: ${error.message}`);
                        } else {
                            delete this.tvdata.program[file.slice(0, 10)];
                        }
                    });
                }
            });
    ...
    this.doStateChange = function (id, state) {
        this.logdebug('doStateChange ${id}');
                    ...
                    data = data.concat(
                        this.tvdata.program[datum].reduce((acc, el) => {
                            ...
                            if (
                                ...
                                new Date(el.startTime) < dend &&
                                new Date(el.endTime) > dstart
                            ) {
                                if (!acc[i]) {
                                    acc[i] = {};
                                }
        ...
        if (typeof msg.message === 'object') {
            ...
            while (dstart <= dend) {
                ...
                if (this.tvdata.program[datum]) {
                    data = data.concat(
                        this.tvdata.program[datum].reduce((acc, el) => {
                            const i = channelfilter.indexOf(el.channel);
                            if (i === -1) {
                                return acc;
                            }
                            ...
                            if (start < dstartfind || end > dendfind) {
                                return acc;
                            }
                            if (categoryfilter.length !== 0 && !categoryfilter.includes(el.content.category)) {
                                return acc;
                            }
                            ...
                            const matchesText = (() => {
                                ...
                                const haystack =
                                    `${el.title ?? ''}${el.content?.texts?.Long?.value ?? ''}${el.content?.texts?.VeryShort?.value ?? ''}`.toLowerCase();
                            ...
                            if (!acc[i]) {
                                acc[i] = { channel: el.channel, events: [], count: 0 };
                            }
                            if (!acc[i].events) {
                                acc[i].events = [];
                            }
                            if (typeof acc[i].count !== 'number') {
                                acc[i].count = 0;
                            }

                            // Enforce maxresults per channel
                            if (acc[i].count < maxresults) {
                                acc[i].channel = el.channel;
                                acc[i].events.push(el);
                                acc[i].count++;
                            }
            ...
            // cleanup: remove empty slots + strip internal counter
            data = data
                .filter(el => el != null)
                .map(el => {
                    if (el && Object.prototype.hasOwnProperty.call(el, 'count')) {
                        const { count, ...rest } = el;
                        return rest;
                    }
                    return el;
    ...
    this.getServerInfoMsg = function (msg) {
        ...
        const data = {};
        data.tvprogram = Object.keys(this.tvdata.program)
            .filter(date => this.tvdata.program[date].length > 0)
            .sort();
        if (msg.callback) {
            this.adapter.sendTo(msg.from, msg.command, data, msg.callback);
        }
    ...
    this.getTVDatapointsAsync = async function () {
        ...
        const id = this.adapter.namespace;
        const options = { startkey: `${id}.`, endkey: `${id}.\u9999`, include_docs: true };
        const objects = await this.adapter.getObjectViewAsync('system', 'state', options);
        return (objects?.rows || [])
            .map(row => row.id)
            .filter(name => name.split('.')[3] === 'config')
        ...
        if (this.adapter.config.tvcount !== tvs.length) {
            for (let i = 1; tvs.length < this.adapter.config.tvcount; i++) {
                if (!tvs.includes(`tv${i}`)) {
                    promises.push(new Promise(resolve => this.createDatapointsPath(`tv${i}`, () => resolve(true))));
                    tvs.push(`tv${i}`);
                }
            }
        }
     ...
     * @param {() => void} callback - The callback function to call when all datapoints are created.
     */
    ...
    this.sourceIdentity = function () {
        this.logdebug(`publishData ${this.adapter.config.source} ${this.adapter.config.country}`);
        return sourceIdentity(this.adapter.config.source, this.adapter.config.country);
    };
    this.saveMetadata = async function () {
        this.logdebug('saveMetadata');
    ...
    /** @param {string[]} tvs - Configured TV identifiers. */
    this.loadCache = async tvs => {
        const identity = this.sourceIdentity();
        this.logdebug(`loadCache ${identity}`);
        let metadata = {};
        try {
            metadata = JSON.parse(await fs.promises.readFile(`${dataDir}metadata.json`, 'utf8'));
        } catch (error) {
            this.logdebug(`Ignoring missing or invalid cache metadata: ${String(error)}`);
        }
        const changed = sourceChanged(metadata.source, identity);
        this.sourceKey = identity;
        this.lastDownload = metadata.source === identity ? metadata.lastDownload || '' : '';
        this.jitterSeed = metadata.jitterSeed || crypto.randomUUID();
        if (changed) {
            this.tvdata = { program: {} };
            for (const file of await fs.promises.readdir(`${dataDir}program/`)) {
                if (/^\d{4}-\d{2}-\d{2}\.json$/.test(file)) {
                    await fs.promises.unlink(`${dataDir}program/${file}`);
                }
            }
            for (const key of ['channels', 'categories', 'genres']) {
                await fs.promises.unlink(`${dataDir}${key}.json`).catch(error => {
            ...
            for (const tv of tvs) {
                await this.setStateAsync('channelfilter', '[]', tv);
                await this.setStateAsync('selectchannel', '', tv);
            }
        }
        if (!changed && (metadata.source === identity || (!metadata.source && identity === 'tvfueralle'))) {
            for (const key of ['channels', 'categories', 'genres']) {
                const data = this.readFileSync(`${key}.json`);
                if (data !== 'error') {
                    this.tvdata[key] = data;
                }
            }
            const files = await fs.promises.readdir(`${dataDir}program/`);
            for (const file of files.filter(name => /^\d{4}-\d{2}-\d{2}\.json$/.test(name))) {
                const data = this.readFileSync(`program/${file}`);
                if (Array.isArray(data)) {
                    this.tvdata.program[file.slice(0, 10)] = data;
                }
            }
        }
        await this.saveMetadata();
    };
    this.publishData = async function (key, value, tvs) {
        this.logdebug(`publishData ${key}`);
        if (JSON.stringify(this.tvdata[key]) === JSON.stringify(value)) {
            return;
        }
        await fs.promises.writeFile(`${dataDir}${key}.json`, JSON.stringify(value));
        this.tvdata[key] = value;
        for (const tv of tvs) {
            this.setState('cmd', `new|${key}`, tv);
        }
    };
    this.publishProgram = async function (date, events, tvs) {
        this.logdebug(`publishProgram ${date}`);
        if (JSON.stringify(this.tvdata.program[date]) === JSON.stringify(events)) {
            return;
        }
        await fs.promises.writeFile(`${dataDir}program/${date}.json`, JSON.stringify(events));
        this.tvdata.program[date] = events;
        for (const tv of tvs) {
            this.setState('cmd', `new|program|${date}`, tv);
        }
    };
    this.getDataAsync = async function (tvs) {
        this.logdebug('getDataAsync');
        const [categories, channels, genres] = await Promise.all([
            this.getCategoriesAsync(),
            this.getChannelsAsync(),
            this.getGenresAsync(),
        ]);
        if (categories.error || channels.error || genres.error) {
            throw new Error('TV für alle metadata request failed');
        }
        await this.publishData('categories', categories.data.category, tvs);
        await this.publishData('channels', channels.data.channels, tvs);
        await this.publishData('genres', genres.data.genres, tvs);
        const date = new Date();
        ...
        for (let i = 0; i < 11; i++) {
            ...
            const result = await this.getProgramAsync(date);
            if (!result.error && Array.isArray(result.data?.events)) {
                await this.publishProgram(
                    this.formatedDate(date),
                    this.shrinkTVProgramWithText(result.data.events),
                    tvs,
                );
            }
    ...
    /** @param {string[]} tvs - Configured TV identifiers. */
    this.scheduleIptv = tvs => {
        this.logdebug('scheduleIptv');
        if (this.stopping) {
            return;
        }
        const now = new Date();
        const today = this.formatedDate(now);
        const runDay = new Date(now);
        if (this.lastDownload === today) {
            runDay.setDate(runDay.getDate() + 1);
        }
        const runDate = this.formatedDate(runDay);
        runDay.setHours(0, 0, 0, -1);
        const jitter = stableId(`${this.jitterSeed}:${runDate}`) / 0x1000000000000;
        const target = nextDownload(runDay, this.adapter.config.loadTime || '03:30', () => jitter);
        const delay = Math.max(0, target.getTime() - now.getTime());
        this.setTimeout('doObserver', () => this.runIptv(tvs, runDate), delay);
    };
    /**
     * @param {string[]} tvs - Configured TV identifiers.
     * @param {string} runDate - Scheduled local date.
     */
    this.runIptv = async (tvs, runDate) => {
        this.logdebug('runIptv');
        try {
            await this.getIptvData(tvs);
            if (this.stopping) {
                return;
            }
            this.lastDownload = runDate;
            await this.saveMetadata();
            this.scheduleIptv(tvs);
        } catch (error) {
            this.logerror(`IPTV-EPG download failed: ${String(error)}`);
            if (!this.stopping) {
                this.setTimeout('doObserver', () => this.runIptv(tvs, runDate), 60 * 60 * 1000);
            }
        }
    };
    /** @param {string[]} tvs - Configured TV identifiers. */
    this.getIptvData = async tvs => {
        this.logdebug('getIptvData');
        const country = this.sourceIdentity().split(':')[1].toLowerCase();
        this.downloadController = new AbortController();
        const response = await axios.get(`https://iptv-epg.org/files/epg-${country}.xml.gz`, {
            responseType: 'stream',
            timeout: 20 * 60 * 1000,
            decompress: false,
            signal: this.downloadController.signal,
        });
        const stream = response.data.pipe(zlib.createGunzip());
        response.data.on('error', error => stream.destroy(error));
        stream.setEncoding('utf8');
        let data;
        try {
            this.logdebug(`parseXmltv`);
            data = await parseXmltv(stream);
        } catch (error) {
        ...
        if (!data.channels.length || !Object.keys(data.program).length) {
            throw new Error('IPTV-EPG returned no channels or programmes');
        }
        await this.publishData('channels', data.channels, tvs);
        await this.publishData('categories', data.categories, tvs);
        await this.publishData('genres', data.genres, tvs);
        for (const date of Object.keys(this.tvdata.program)) {
            if (!Object.hasOwn(data.program, date)) {
                await this.publishProgram(date, [], tvs);
            }
        }
        for (const [date, entries] of Object.entries(data.program)) {
            await this.publishProgram(date, entries, tvs);
        }
        this.removeFiles();
    };
    /**
    ...
    this.getProgramAsync = async function (datum) {
        ...
        try {
            return await tvfueralle.getProgram(options['%DATE%']);
        } catch (error) {
    ...
    this.getCategoriesAsync = async function () {
        ...
        try {
            return await tvfueralle.getCategories();
        } catch (error) {
            this.logerror(JSON.stringify(error));
            return { error: true };
        }
    ...
    this.getChannelsAsync = async function () {
        ...
        try {
            return await tvfueralle.getChannels();
        } catch (error) {
            this.logerror(JSON.stringify(error));
            return { error: true };
        }
    ...
    this.getGenresAsync = async function () {
        ...
        try {
            return await tvfueralle.getGenres();
        } catch (error) {
            this.logerror(JSON.stringify(error));
            return { error: true };
        }
     ...
     * @param {(...args: unknown[]) => void} callback - The function to call after attempting to create the object.
     */
     ...
     * @param {string} level1path - The first level path segment for the object name.
     * @param {string} level2path - The second level path segment for the object name.
     * @param {(...args: unknown[]) => void} callback - The function to call with the retrieved state object.
     */
    ...
    this.writeFile = function (filename, data, callback = undefined) {
        this.logdebug(`writeFile ${filename}`);
    ...
    this.readFileSync = function (filename) {
        this.logdebug(`readFileSync ${filename}`);
        let data;
        try {
     ...
     * @param {() => void} callback - The function to be executed after the timeout period.
