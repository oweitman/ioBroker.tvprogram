...
function tvprogramclass(adapter) {
    ...
    this.getServerDataMsg = function (msg) {
        ...
        this.logdebug("getServerDataMsg send" + msg.from + " " + msg.command + " " + JSON.stringify(data).substring(0, 100) + " " + msg.callback);
    ...
    this.createDatapointsPath = function (path, callback) {
        ...
        const pName = new Promise((resolve) => {
            ...
            this.createObjectNotExist(stateTemplatename, path, null, function (resolve) {
                that.setState(stateTemplatename.name, path, path);
                resolve();
            }.bind(this, resolve));
        });
        const pConfig = new Promise((resolve) => {
            ...
            this.createObjectNotExist(stateTemplateconfig, path, null, function (resolve) {
                that.setState(stateTemplateconfig.name, "{}", path);
                resolve();
            }.bind(this, resolve));
        });
        const pFavorites = new Promise((resolve) => {
            ...
            this.createObjectNotExist(stateTemplatefavorites, path, null, function (resolve) {
                that.setState(stateTemplatefavorites.name, "[]", path);
                resolve();
            }.bind(this, resolve));
        });
        const pChannelfilter = new Promise((resolve) => {
            ...
            this.createObjectNotExist(stateTemplatechannelfilter, path, null, function (resolve) {
                that.setState(stateTemplatechannelfilter.name, "[]", path);
                resolve();
            }.bind(this, resolve));
        });
        const pShow = new Promise((resolve) => {
            ...
            this.createObjectNotExist(stateTemplateshow, path, null, function (resolve) {
                that.setState(stateTemplateshow.name, "1", path);
                resolve();
            }.bind(this, resolve));
        });
        const pChannel = new Promise((resolve) => {
            ...
            this.createObjectNotExist(stateTemplateselch, path, null, function (resolve) {
                that.setState(stateTemplateselch.name, "", path);
                resolve();
            }.bind(this, resolve));
        });
        const pCmd = new Promise((resolve) => {
            ...
            this.createObjectNotExist(stateTemplateselcmd, path, null, function (resolve) {
                that.setState(stateTemplateselcmd.name, "", path);
                resolve();
            }.bind(this, resolve));
        });
        const pRecord = new Promise((resolve) => {
            ...
            this.createObjectNotExist(stateTemplateselrecord, path, null, function (resolve) {
                that.setState(stateTemplateselrecord.name, "", path);
                resolve();
            }.bind(this, resolve));
        });
    ...
    this.getData = function (tvs) {
        ...
        while (dstart <= dend) {
            this.getProgram(dstart, function (datum, data) {
                if (data === "error") {
                    data = that.readFileSync("program/" + datum + ".json");
                    if (data === "error") data = { events: [] };
                }
                data.events = that.shrinkTVProgramWithText(data.events);
                if (JSON.stringify(that.tvdata.program[datum]) !== JSON.stringify(data.events)) {
                    that.logdebug("getData first request or changed data events " + datum);
                    that.writeFile("program/" + datum + ".json", data);
                    that.tvdata.program[datum] = data.events;
                    tvs.map((tv) => that.setState("cmd", "new|program|" + datum, tv));
                }
